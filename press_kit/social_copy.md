...
- Perf multipliers: DeepSeek (10×, per-user vs published NVIDIA records) and LFM2.5 (10,000 tok/s) are LIVE with citations in `launch_playbook.md` §1. Headlines/titles carry the bare multiplier; the linked body's FIRST paragraph carries the scoping (design ceiling at the planning clock vs measured hardware, re-runnable model, workload, system size). WAN/video figures are ON HOLD pending open `VSA_CASE_STUDIES:suspected_mistakes_ledger.md` entries — no video-throughput claims until resolved.
