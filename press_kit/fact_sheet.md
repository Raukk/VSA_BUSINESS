...
| Per-user decode rate | **over 1,000 tok/s/user**: the claimed floor across modeled configurations, since the rate is model- and setup-dependent (owner ruling 2026-08-19). Per-configuration ceilings, quotable only with the configuration named: 4,096 tok/s/user DeepSeek-V3/R1 class at 8,192 concurrent users, 262,144 tok/s/user LFM2.5-class small model on a single wafer. Structural bandwidth argument; simplifications acknowledged; competitor-anchored benchmark queued | **[estimate: designer's analysis]** floor consistent with [`VSA_CASE_STUDIES:deepseek/llm_decode_speed_estimate_2026-06-11.md` via `productization_feasibility_2026-07-10.md` §9.2]; ceilings [`VSA_CASE_STUDIES:deepseek/dgx_h200_comparison_2026-07-16.md`, `VSA_CASE_STUDIES:lfm25/lfm25_on_hyper_evaluation.md` via `press_kit/launch_playbook.md` §1] |
