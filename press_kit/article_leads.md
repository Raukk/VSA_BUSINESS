...
> Enterprises say their agents need to answer in under half a second, and half of deployments miss that at peak load. Here is why more GPUs don't close it: between an agent's tool calls sits a model generating tokens strictly one after another, and you cannot parallelize token four across two racks. The best per-user rate NVIDIA has ever published is just over 250 tokens per second, which spends a 500-millisecond budget on 125 tokens; its published p99 under MLPerf serving conditions is 12.5 per second, which spends it on six. The VSA's modeled schedules deliver over 1,000 tokens per second to a single user while thousands of others are served concurrently, and that is the floor across configurations, not the ceiling. Because execution is fully deterministic there is also no tail: the p99 is the p50. Design ceilings against measured hardware, and every schedule behind them is public. [`VSA_CASE_STUDIES:deepseek/dgx_h200_comparison_2026-07-16.md`, `VSA_CASE_STUDIES:lfm25/lfm25_on_hyper_evaluation.md`, `VSA_ASIC:docs/baseline/core_design.md` §4; rate floor per owner ruling 2026-08-19 and external latency survey figures per `press_kit/response_agentic_latency_2026-08-19.md` §4]
