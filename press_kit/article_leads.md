...
**Status:** Draft. Every launch headline links to an article that opens with the universal lead below — verbatim or lightly adapted — then the paragraph matched to the headline that was clicked. The lead flows as prose that happens to carry the load-bearing facts (design status, re-runnable model) — it is a story opening, not a disclosure block (owner direction 2026-08-14). Replace `[REPO_URL]` before publishing.

---

## The universal lead (paragraph one, everywhere)

> This morning an AI chip design went public — all of it: the specifications, the compiler, and a bit-exact model of exactly what the silicon would do. It's called the VSA, it's free for anyone to run, fork, build, or sell — royalty-free, forever — and nobody has built it yet. That last part isn't a catch; it's the design. Execution is deterministic down to the cycle, so the model doesn't approximate the chip — it defines it, and every number in this article is math you can re-run yourself tonight. [REPO_URL]
