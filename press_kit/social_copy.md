...
>
> The core bet: trade peak MACs-per-cycle for massive on-chip weight residency, so whole networks chain layer-to-layer on chip with no HBM and no external-RAM round-trips.
>
...
> **1/** Today I'm open-sourcing an AI inference chip design: the VSA (Virtual Systolic Array). Specs, golden model, compiler, and a bit-exact end-to-end demo. Free to build, forever. [REPO_URL]
>
> **2/** The bet: instead of chasing peak MACs-per-cycle, keep weights resident on chip and chain whole networks layer-to-layer, with no HBM and no external-RAM round-trips. In 2026, "no HBM" is a supply-chain feature, not a bug.
>
...
- Perf multipliers: DeepSeek (10×, per-user vs published NVIDIA records) and LFM2.5 (10,000 tok/s) are LIVE with citations in `launch_playbook.md` §1. Headlines/titles carry the bare multiplier; the linked body's FIRST paragraph carries the scoping (design ceiling at the planning clock vs measured hardware, re-runnable model, workload, system size). WAN/video figures are LIVE (owner ruling 2026-08-14; cites in `launch_playbook.md` §1): ~1 clip/s power-limited wafer, H100 ~12–24 min/clip, ~100–200× per dollar vs best measured platform. Cost hooks (1/10th and better) are LIVE with the DeepSeek/LFM cost cites in `launch_playbook.md` §1.
